...
import { AlertCircle } from 'lucide-react'; // Using AlertCircle as the fail icon
import spanner from "../assets/spanner.svg"
const ScanFailModal = ({ isOpen, onClose, onRetry }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
      ...
      <div className="relative w-full max-w-sm bg-white rounded-[2rem] px-6 pb-6 pt-16 text-center shadow-2xl animate-in fade-in zoom-in duration-300">
        ...
        <div className="absolute -top-10 left-1/2 -translate-x-1/2 flex size-20 items-center justify-center rounded-[1.5rem] bg-gradient-to-b from-white to-[#FDCED2] shadow-sm">
           {/* You can replace this Icon with your own <img> if needed */}
         <img src={spanner} className='size-12' ></img>
        </div>
        
        {/* Title Text (Red) */}
        <h2 className="text-2xl font-bold text-[#EB0A1E]">Scanning Failed</h2>
        <p className="mt-2 text-gray-500 mb-8">QR code scanning failed, Try again.</p>

        {/* Buttons Container */}
        <div className="flex flex-col gap-4">
            
            {/* PRIMARY BUTTON: Scan Again (Dark Blue) */}
            <button 
                onClick={onRetry}
